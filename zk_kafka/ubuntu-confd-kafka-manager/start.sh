...
ZK_SERVICE=${ZK_SERVICE:-"ConfdHDP/zk"}

export CONFD_BACKEND CONFD_PREFIX CONFD_INTERVAL CONFD_PARAMS ZK_SERVICE
...
if [ "$CONFD_INTERVAL" -gt "0" ]; then
    ...
    echo "/usr/bin/nohup /usr/bin/confd ${CONFD_PARAMS} > /opt/kafka-manager/logs/confd.log 2>&1 &" >> ${CONFD_SCRIPT}
